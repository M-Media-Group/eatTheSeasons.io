...
class CustomNotification {
  ...
  public show() {
    ...
    if (this.modes.includes("push")) {
      new PushNotification("EatNinja", this.message).show();
    }
...
export default {
  consumed_food_item_add: (event: FoodItem) => {
    const allowedCalories =
      (store.getters["auth/nextMeal"]?.totalAllowedCalories ?? 0) -
        store.getters["consumedItems/kcalConsumedToday"] >
      0
        ? Math.round(
            store.getters["auth/nextMeal"].totalAllowedCalories -
              store.getters["consumedItems/kcalConsumedToday"]
          )
        : 0;
